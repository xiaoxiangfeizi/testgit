...
(function($){
	...
	var defaults = {
		...
		selectorModalboxCloseContainer 		: '#modalBoxHeader',
		...
		*/
		setModalboxLayoutContainer_Begin	: '<div class="modalBoxBodyContent">',
		setModalboxLayoutContainer_End		: '</div>',
	...
	var methods = {
		...
		init : function( globaloptions ) {
			...
			function openModalBox(settings){
				...
				if( settings.loadingImagePreparer["currentImageObj"] && !settings.loadingImagePreparer["finalizeModalBox"] ){
					...
					if( settings.type && globaloptions.callFunctionBeforeShow() ){
                        ...
                        //
                        $('.tabs').click(function(){
                          obj = jQuery(globaloptions.selectorModalboxContainer);
                          height = obj.height()+parseInt(obj.css('top'));
                          if(jQuery(window).height()<height && $(document).scrollTop()==0){ obj.css({ position	: 'absolute', display		: "block",visibility	: "visible"}); }
                        });
			...
			function centerModalBox(settings){
				...
				if( jQuery(globaloptions.selectorPreCacheContainer).length == 0 && modalboxContainerObj.length != 0 ){
                  ...
                  handle: 'div#modalBoxHeader',
                  cancel: '.closeModalBox'
								});
							}
		...
		centerModalBox : function(settings){
		
		  var settings = jQuery.extend({}, defaults, settings);
		  
		  var containerObj = jQuery(settings.selectorModalboxContainer);		  
			var getModalboxContainerHeight = containerObj.height();
  		...
  		containerObj.css({top:setPositionTop});																	  
		},
		...
		modalboxBuilder : function(settings){
			...
			createModalboxContainer += '<div id="' + prepareNameOfModalboxContainer + '"' + settings.customStyles + '>';
				createModalboxContainer += '<div id="' + prepareNameOfModalboxBodyContainer + '">';
					...
					createModalboxContainer += '<div id="' + prepareNameOfCloseButtonContainer + '"><a href="javascript:void(0);" class="closeModalBox"><div class="closeModalBoxButton"></div></a></div>';
