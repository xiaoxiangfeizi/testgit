...
<form method="post" action="<?php echo url_for('tasks/export') ?>">
<?php if($sf_request->hasParameter('projects_id')) echo input_hidden_tag('projects_id',$sf_request->getParameter('projects_id')) ?>
<div id="extport_fields"><?php echo select_tag('fields',array('name','TasksStatus'),array('choices'=>$columns,'expanded'=>true,'multiple'=>true)) ?></div><br>

<div><?php echo __('Filename') . ': ' .  input_tag('filename',__('Tasks'),array('size'=>40)) ?></div><br>
