...
{
  $f = new sfWidgetFormInput();
  return $f->render('',$value,array_merge($attributes,array('type'=>'button','class'=>'btn','onClick'=>'location.href="' . url_for($url) . '"')));
}

function submit_tag($value='',$attributes = array())
{
  $f = new sfWidgetFormInput();
  return $f->render('',$value,array_merge($attributes,array('type'=>'submit','class'=>'btn')));
}
